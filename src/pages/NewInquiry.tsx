...
export default function NewInquiry() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "문의 저장에 실패했어요.",
      );
    } finally {
    ...
    <main className="relative min-h-dvh w-full overflow-hidden bg-white px-5 pb-12 pt-6">
      ...
      <section className="mt-10 space-y-6">
        <label className="block">
          <span className="text-base font-normal leading-6 text-brown-600">
            문의 유형
          </span>
          <select
            className="mt-3 h-14 w-full rounded-2xl border border-cream-200 bg-cream-50 px-4 text-base font-normal leading-6 text-brown-600 outline-none"
            value={category}
        ...
        <label className="block">
          ...
          <input
            className="mt-3 h-14 w-full rounded-2xl border border-cream-200 bg-cream-50 px-4 text-base font-normal leading-6 text-brown-600 outline-none placeholder:text-[#b9aaa4]"
            value={title}
        ...
        <label className="block">
          ...
          <textarea
            className="mt-3 min-h-48 w-full resize-none rounded-2xl border border-cream-200 bg-cream-50 px-4 py-4 text-base font-normal leading-7 text-brown-600 outline-none placeholder:text-[#b9aaa4]"
            value={content}
        ...
        <button
          type="button"
          className="flex h-16 w-full items-center justify-center gap-3 rounded-full bg-brown-600 text-xl font-normal leading-7 text-white shadow-lg disabled:opacity-60"
