    ...
    <main className="min-h-dvh bg-white px-5 pb-12 pt-6">
      ...
      <section className="mt-10 rounded-3xl bg-cream-50 px-5 py-5">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 text-sm font-normal leading-5">
          <button className="rounded-full bg-brown-600 px-4 py-2 text-white">
            회원 관리
          </button>
          <button
            type="button"
      ...
      <section className="mt-6 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          >
            <option value="all">role 전체</option>
          ...
          <select
            ...
            value={toneFilter}
            onChange={(event) =>
              setToneFilter(event.target.value as ToneCode | "all")
            }
          >
          ...
          <select
            ...
            onChange={(event) =>
              setWaitlistFilter(
                event.target.value as "all" | "joined" | "not_joined",
              )
            }
          >
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate text-xs font-normal leading-5 text-[#9b8179]">
                    {user.id}
                  </p>
                  <p className="mt-3 text-sm font-normal leading-5 text-[#7a625c]">
                    가입 {formatDate(user.createdAt)} · 찜{" "}
                    {user.savedProductCount}개
                  </p>
                  <p className="mt-1 text-sm font-normal leading-5 text-[#7a625c]">
                    최근 톤{" "}
                    {user.latestToneLabel ?? user.latestToneCode ?? "진단 없음"} ·{" "}
                    {formatConfidence(user.latestConfidence)}
                  </p>
                </div>
                <HiChevronRight
