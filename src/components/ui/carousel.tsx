...
const CarouselItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  ...
  return <div ref={ref} role="group" aria-roledescription="slide" className={cn("min-w-0 shrink-0 grow-0 basis-full", orientation === "horizontal" ? "pl-0" : "pt-0", className)} {...props} />;
});
...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ref={ref}
      type="button"
      className={cn("flex size-11 items-center justify-center rounded-full border border-[#E8D7B2] text-[#3A2527] transition disabled:pointer-events-none disabled:opacity-40", className)}
      disabled={!canScrollPrev}
      onClick={scrollPrev}
      {...props}
    >
      <HiArrowRight className="size-5 rotate-180" aria-hidden="true" />
